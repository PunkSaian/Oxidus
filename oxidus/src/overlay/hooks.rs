...
use crate::overlay::{sdl_renderer::Renderer, OverlayState};

use super::{scan_code_map::sdl_scancode_to_imgui_key, OVERLAY_STATE};
...
pub unsafe extern "C" fn poll_event(event: *mut c_void) -> i32 {
    ...
    if result != 0 {
        ...
        if let Some(OverlayState { context, .. }) = state.as_mut() {
