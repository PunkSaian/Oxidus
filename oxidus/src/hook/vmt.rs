...
static HOOK_REGISTRY: OnceLock<RwLock<HashMap<FnPtr, FnPtr>>> = OnceLock::new();

fn get_registry() -> &'static RwLock<HashMap<FnPtr, FnPtr>> {
    HOOK_REGISTRY.get_or_init(|| RwLock::new(HashMap::new()))
}
...
impl VmtHook {
    pub unsafe fn new(vmt: *mut *mut (), index: usize, hook_fn: *mut ()) -> OxidusResult<Self> {
        let page_size = libc::sysconf(libc::_SC_PAGESIZE) as usize;
        let vmt_ptr = vmt as usize;
        let aligned_addr = vmt_ptr & !(page_size - 1);
        let protect_size = (vmt_ptr - aligned_addr) + std::mem::size_of::<*mut ()>() * (index + 1);
        ...
        if mprotect(
            ...
            return Err(OxidusError::Hooking("Failed to change memory protection".into()));
        }
